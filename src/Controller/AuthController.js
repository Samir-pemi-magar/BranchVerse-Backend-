...
exports.Signup = async (req, res) => {
  try {
    ...
    try {
      ...
      console.log("Verification email sent");  // ← add this
    } catch (emailErr) {
      console.error("EMAIL ERROR:", emailErr.message);  // ← add this
      // Still return success so user knows account was created
      return res.status(201).json({
        msg: "Signup successful but email failed. Contact support.",
        user
      });
    }
  ...
  try {
    ...
    res.send(`
      ...
      <head>
        ...
        <style>
          body { display:flex; justify-content:center; align-items:center; height:100vh; font-family:'Segoe UI',Tahoma,Geneva,Verdana,sans-serif; background:#ffffff; margin:0; }
          .container { background:#fff; padding:40px 60px; border-radius:12px; box-shadow:0 10px 25px rgba(0,0,0,0.2); text-align:center; }
          h1 { color:#10B981; margin-bottom:20px; }
          p { color:#374151; font-size:18px; }
          a { display:inline-block; margin-top:25px; padding:10px 25px; background:#3B82F6; color:white; border-radius:6px; text-decoration:none; font-weight:bold; transition:0.3s; }
      ...
      <body>
        <div class="container">
          ...
          <a href="${process.env.FRONTEND_URL}/auth/login">Go to Login</a>  <!-- ✅ fixed -->
        </div>
      </body>
      </html>
    `);
    ...
    res.send(`
      ...
      <head>
        ...
        <style>
          ...
          p { color:#374151; font-size:18px; }
          a { display:inline-block; margin-top:25px; padding:10px 25px; background:#EF4444; color:white; border-radius:6px; text-decoration:none; font-weight:bold; transition:0.3s; }
      ...
      <body>
        <div class="container">
          ...
          <a href="${process.env.FRONTEND_URL}/signup">Try Again</a>  <!-- ✅ fixed -->
        </div>
      </body>
      </html>
    `);
